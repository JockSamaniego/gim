...
@Entity
@TableGenerator(
		name = "StatusChangeGenerator", 
		table = "IdentityGenerator", 
		pkColumnName = "name", 
		valueColumnName = "value", 
		pkColumnValue = "StatusChange", 
...
//			query = "select NEW org.gob.gim.income.dto.BondDownStatus(sc.explanation, sum(sc.municipalBond.value+sc.municipalBond.taxesTotal-sc.municipalBond.discount)) " +
			query = "select NEW org.gob.gim.income.dto.BondDownStatus(sc.explanation, sum(sc.municipalBond.value+sc.municipalBond.taxesTotal)) " +
				...
				"group by sc.explanation " +
				"order by sc.explanation"),
	@NamedQuery(name = "StatusChange.findByResolution", 
	...
	@NamedQuery(name = "StatusChange.findBondsDownStatusByResolution", 
		query = "select mb from StatusChange sc " +
				"left join sc.municipalBond mb " +
				"left join fetch mb.receipt receipt " +
				"left join fetch mb.resident resident " +
				"where sc.municipalBondStatus.id=9 " +
				"and sc.explanation = :resolution " +
				"and sc.date = mb.reversedDate " +
				"order by mb.emisionDate"),
	@NamedQuery(name = "StatusChange.findBondsDownStatusByDates", 
		query = "select mb from StatusChange sc " +
				"left join sc.municipalBond mb " +
				"left join fetch mb.receipt receipt " +
				"left join fetch mb.resident resident " +
	...
	@NamedQuery(name = "StatusChange.findBondsByStatusAndDate", 
			query = "select distinct(mb) from StatusChange sc " +
				"left join sc.municipalBond mb " +
				"left join fetch mb.receipt receipt " +
				"left join fetch mb.resident resident " +
				"where sc.municipalBondStatus.id=:statusId " +
				"and sc.date between :startDate and :endDate " +
				"order by mb.reversedResolution, mb.reversedDate, mb.emisionDate"),
				
	@NamedQuery(name = "StatusChange.findBondsByStatusAndDateAndUser", 
		query = "select distinct(mb) from StatusChange sc " +
			"left join sc.municipalBond mb " +
			"left join fetch mb.receipt receipt " +
			"left join fetch mb.resident resident " +
	...
	@NamedQuery(name = "StatusChange.findDownBondsWithAccountByResolutionOriginal", 
		query = "select NEW org.gob.gim.income.dto.BondDownStatus(ac.accountCode, ac.accountName, sum(i.total)) from StatusChange sc " +
				"left join sc.municipalBond mb " +
				"left join mb.items i " +
				"left join i.entry e " +
				...
				"group by ac.accountCode, ac.accountName " +
				"order by ac.accountCode"),
	@NamedQuery(name = "StatusChange.findDownBondsWithAccountByResolution", 
		query = "select NEW org.gob.gim.income.dto.BondDownStatus(ac.accountCode, ac.accountName, sum(i.total)) from StatusChange sc " +
				"left join sc.municipalBond.items i " +
				"left join i.entry.account ac " +
				"where sc.municipalBondStatus.id=9 " +
				"and sc.explanation = :resolution " +
				"group by ac.accountCode, ac.accountName " +
				"order by ac.accountCode"),
	@NamedQuery(name = "StatusChange.findDownBondsWithAccountByDates", 
		query = "select NEW org.gob.gim.income.dto.BondDownStatus(ac.accountCode, ac.accountName, sum(i.total)) from StatusChange sc " +
				"left join sc.municipalBond.items i " +
				"left join i.entry.account ac " +
				"where sc.municipalBondStatus.id=9 " +
	...
	@NamedQuery(name = "StatusChange.findLastChangeByMunicipalBond", 
		query = "select sc from StatusChange sc " +
				"where sc.municipalBond = :municipalBond " +
				"order by sc.date+sc.time desc limit 1"
				)
...
public class StatusChange {
	...
	public String toString() {
		return "StatusChange [id=" + id + ", date=" + date + ", time=" + time
				+ ", explanation=" + explanation + ", user=" + user
				+ ", previousBondStatus=" + previousBondStatus
				+ ", municipalBondStatus=" + municipalBondStatus
				+ ", municipalBond=" + municipalBond + ", totalForResolution="
