...
public class ResidentServiceBean implements ResidentService {
	...
	public Resident find(Long id) {
		...
		List<Resident> result = crudService.findWithNamedQuery(
				"Resident.findById", parameters, 1);
		if (!result.isEmpty())
			return result.get(0);
		return null;
	}
	 ...
	 */
	@Override
	public Resident find(String identificationNumber)
	...
	public List<Resident> findByCriteria(String criteria) {
		...
		return crudService.findWithNamedQuery("Resident.findByCriteria",
				parameters);
	}

	@Override
	public void verifyUniqueness(Long id, String identificationNumber,
			IdentificationType identificationType)
			throws IdentificationNumberExistsException {
		try {
	...
	@SuppressWarnings("unchecked")
	private Resident findByDistinctId(Long id, String identificationNumber)
			throws NonUniqueIdentificationNumberException {
		try {
			Query query = em
					.createNamedQuery("Resident.findDistinctByIdentificationNumberAndId");
			query.setParameter("identificationNumber", identificationNumber);
	...
	public void save(Resident resident)
		...
		if (resident.getId() != null) {
			try {
				...
				if (r != null) {
					if (r.getId().longValue() == resident.getId().longValue()) {
						...
						// System.out.println("=======> ACTUALIZADO CONTRIBUYENTE CORRECTAMENTE con ids true, DNI:  "
						// + resident.getIdentificationNumber());
					} else
						throw new IdentificationNumberExistsException();
				} else {
					crudService.update(resident);
					// System.out.println("=======> ACTUALIZADO CONTRIBUYENTE CORRECTAMENTE: "
					// + resident.getIdentificationNumber());
					// throw new IdentificationNumberExistsException();
				}
			} catch (NonUniqueIdentificationNumberException e) {
			...
			if (resident.getIdentificationType() == IdentificationType.GENERATED_NUMBER) {
				Query query = em
						.createNativeQuery("SELECT nextval('residentNumber')");
				String generatedNumber = ((BigInteger) query.getResultList()
						.get(0)).toString();
				resident.setIdentificationNumber(generatedNumber);
			} else {
				verifyUniqueness(resident.getId(),
						resident.getIdentificationNumber(),
						resident.getIdentificationType());
			}
			crudService.create(resident);
			// System.out.println("=======> INSERTADO CONTRIBUYENTE : " +
			// resident.getIdentificationNumber());
		}
	}
	...
	public void loadPublicInstitutions() {
		...
		Query query = this.em
				.createNativeQuery("select CAST ( id AS integer ) "
						+ "from resident " + "where residenttype = 'J' "
	 ...
	 * 
	 * @see
	 * org.gob.gim.common.service.ResidentService#updatePublicInstitutions()
	 */
	@Override
	...
	public Resident saveResident(Resident resident)
			throws IdentificationNumberExistsException {
		return crudService.update(resident);
	}
