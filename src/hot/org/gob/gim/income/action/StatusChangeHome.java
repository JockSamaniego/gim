...
public class StatusChangeHome extends EntityHome<StatusChange> {
	...
	public List<StatusChangeItem> fillMunicipalBondItems(List<MunicipalBond> municipalBonds) {
		...
		for (MunicipalBond municipalBond : municipalBonds) {
			...
			// if
			// (municipalBond.getMunicipalBondStatus().equals(municipalBondStatus))
			// {
			// municipalBond.setTotalCancelled(BigDecimal.ZERO);
			// }
			// if
			// (municipalBond.getMunicipalBondStatus().equals(inAgreementStatus))
			// {
		...
		for (StatusChangeItem mbi : root.getMunicipalBondItems("g")) {
			// System.out.println("call ->> " + i + "size >>>" +
			// mbi.getChildren().size());
	...
	public void setStatusChangeListGroup(List<StatusChangeItem> statusChangeListGroup) {
	...
	public void setBondsDownStatusByAccount(List<BondDownStatus> bondsDownStatusByAccount) {
	...
	/**
	 * @return the reverseActive
	 */
	public Boolean getReverseActive() {
		return reverseActive;
	}

	/**
	 * @param reverseActive
	 *            the reverseActive to set
	 */
	...
	public List<StatusChange> findAllStatusChangeGrouped() {
		Query query = this.getEntityManager().createNamedQuery("StatusChange.findAllGrouped");
		query.setParameter("startDateFiscalPeriod", userSession.getFiscalPeriod().getStartDate());
		query.setParameter("endDateFiscalPeriod", userSession.getFiscalPeriod().getEndDate());
	...
	public void loadBondDownStatusByDates() {
		...
		bondsDownStatus.clear();
		Query query = this.getEntityManager().createNamedQuery("StatusChange.findAllGroupedByDates");
		query.setParameter("startDate", startDate);
		query.setParameter("endDate", endDate);
		bondsDownStatus = query.getResultList();
		String initDate = dateToStr(startDate);
		String finalDate = dateToStr(endDate);
		String todayDate = dateToStr(today);
		if ((initDate.equalsIgnoreCase(finalDate)) && (initDate.equalsIgnoreCase(todayDate)))
			reverseActive = true;
		else
			reverseActive = false;
	...
	public void findBondDownStatus() {
		...
		total = BigDecimal.ZERO;
		bondsDownStatus.clear();
		Query query = this.getEntityManager().createNamedQuery("StatusChange.findAllGroupedByResolution");
	...
	public void findBondsDownStatusByResolution(BondDownStatus bondDownStatus) {
		...
		this.bondDownStatus = bondDownStatus;
		Query query = this.getEntityManager().createNamedQuery("StatusChange.findBondsDownStatusByResolution");
	...
	public void findBondsDownStatusByDates() {
		// System.out .println("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr ingreso
		// a findBondsDownStatusByDates");
		total = BigDecimal.ZERO;
		bonds.clear();
		bondsDownStatusByAccount.clear();
		// System.out .println("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr clear
		// bonds");
		Query query = this.getEntityManager().createNamedQuery("StatusChange.findBondsDownStatusByDates");
		query.setParameter("startDate", startDate);
		query.setParameter("endDate", endDate);
		// System.out .println("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr prepare
		// query");
		bonds = query.getResultList();
		// int maxResults = 1000;
		...
		// }
		// System.out.println("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr excecute
		// query");
		for (MunicipalBond row : bonds) {
			// total =
			// total.add(row.getValue()).add(row.getTaxesTotal()).subtract(row.getDiscount());
		...
		// System.out .println("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr exit
		// procedure");
	}

	@SuppressWarnings("unchecked")
	public void findDownBondsWithAccountByResolution(BondDownStatus bondDownStatus) {
		total = BigDecimal.ZERO;
		bonds.clear();
		bondsDownStatusByAccount.clear();
		this.bondDownStatus = bondDownStatus;
		String sqlNative = "select ac.accountcode, ac.accountname, sum(i.total) from gimprod.StatusChange sc "
				+ "left join gimprod.municipalbond mb " + "on mb.id=sc.municipalbond_id " + "left join gimprod.item i "
				+ "on i.municipalbond_id=sc.municipalbond_id " + "left join gimprod.entry e " + "on e.id = i.entry_id "
				+ "left join gimprod.account ac " + "on ac.id = e.account_id " + "where sc.municipalbondstatus_id = 9 "
				+ "and sc.explanation = '" + bondDownStatus.getResolutionNumber() + "'" + "and mb.emisionDate >= '"
		...
		for (Object[] row : results) {
			BondDownStatus parte = new BondDownStatus((String) row[0], (String) row[1], (BigDecimal) row[2]);
		...
		sqlNative = "select ac.previousyearsaccountcode, ac.accountname, sum(i.total) from gimprod.StatusChange sc "
				...
				+ "and sc.explanation = '" + bondDownStatus.getResolutionNumber() + "'" + "and mb.emisionDate < '"
				+ dateToStr(userSession.getFiscalPeriod().getStartDate()) + "'"
				+ "group by ac.previousyearsaccountcode, ac.accountname " + "order by ac.previousyearsaccountcode";
		...
		// results.clear();
		// sqlNative =
		// "select ac.accountcode, ac.accountname, sum(i.total) from
		// gimprod.StatusChange sc "
		// +
		...
		// results.clear();
		// sqlNative =
		// "select ac.previousyearsaccountcode, ac.accountname, sum(i.total)
		// from gimprod.StatusChange sc "
		// +
		// "left join gimprod.municipalbond mb " +
		// "on mb.id=sc.municipalbond_id " +
		// "left join gimprod.item i " +
		...
		sqlNative = "select ac.accountcode, ac.accountname, sum(ti.value) from gimprod.StatusChange sc "
				+ "left join gimprod.municipalbond mb " + "on mb.id=sc.municipalbond_id "
				+ "left join gimprod.taxitem ti " + "on ti.municipalbond_id=sc.municipalbond_id "
				+ "left join gimprod.tax t " + "on t.id = ti.tax_id " + "left join gimprod.account ac "
				+ "on ac.id = t.taxaccount_id " + "where sc.municipalbondstatus_id = 9 " + "and sc.explanation = '"
				+ bondDownStatus.getResolutionNumber() + "'" + "group by ac.accountcode, ac.accountname "
				+ "order by ac.accountCode ";
	...
	public void findDownBondsWithAccountByDates() {
		...
		String sqlNative = "select ac.accountcode, ac.accountname, sum(i.total) from gimprod.StatusChange sc "
				...
				+ "on i.municipalbond_id=sc.municipalbond_id " + "left join gimprod.entry e " + "on e.id = i.entry_id "
				+ "left join gimprod.account ac " + "on ac.id = e.account_id " + "where sc.municipalbondstatus_id = 9 "
				+ "and sc.date between '" + dateToStr(startDate) + "' AND '" + dateToStr(endDate) + "' "
				+ "and mb.emisionDate >= '" + dateToStr(userSession.getFiscalPeriod().getStartDate()) + "' "
				+ "group by ac.accountcode, ac.accountname " + "order by ac.accountCode";
		...
		sqlNative = "select ac.accountcode, ac.accountname, sum(ti.value) from gimprod.StatusChange sc "
				...
				+ "left join gimprod.taxitem ti " + "on ti.municipalbond_id=sc.municipalbond_id "
				+ "left join gimprod.tax t " + "on t.id = ti.tax_id " + "left join gimprod.account ac "
				+ "on ac.id = t.taxaccount_id " + "where sc.municipalbondstatus_id = 9 " + "and sc.date between '"
	...
	public String dateToStr(Date date) {
		...
		cal.setTime(date);
		str = String.valueOf(cal.get(Calendar.YEAR)) + "-" + String.valueOf(cal.get(Calendar.MONTH) + 1) + "-"
	...
	public void searchResidentPrevious() {
		...
		query.setParameter("identificationNumber", this.identificationNumberPrevious);
		try {
	...
	public void searchResidentCurrent() {
		Query query = getEntityManager().createNamedQuery("Resident.findByIdentificationNumber");
		query.setParameter("identificationNumber", this.identificationNumberCurrent);
		try {
			Resident resident = (Resident) query.getSingleResult();
	...
	public void searchMunicipalBond() {
		...
		if (previousResident != null) {
			String sentence = "select mb from MunicipalBond mb " + "left join FETCH mb.entry entry "
					+ "left join FETCH mb.resident resident " + "left join FETCH mb.deposits deposit "
					+ "left join FETCH mb.receipt receipt " + "LEFT JOIN FETCH resident.currentAddress "
					+ "where mb.resident.id=:rId " + "and mb.municipalBondStatus.id = 3"
	...
	public String startTransfering() {
		...
		// System.out.println("statusChangeListGroup size >> " +
		// statusChangeListGroup.size());
		// if (statusChangeListGroup.size() > 0) {
		if (statusChangeLst.size() > 0) {
			for (StatusChange sc : statusChangeLst) {
				// System.out.println("sc getChangeResident >> " +
				// sc.getChangeResident());
			...
			// System.out.println("tc getChangeResident >> " +
			// tc.getChangeResident());
	...
	public void insertStatusChange() {
		...
		for (StatusChange sc : statusChangeLst) {
			...
			if (sc.getChangeResident()) {
				explanation = this.getInstance().getExplanation() + "\n Dueño anterior c.i. = "
	...
	public void setBondsWithCorrectionDTO(List<MunicipalBondErrorsCorrectionDTO> bondsWithCorrectionDTO) {
	...
	public void setBondsWithCorrectionAccount(List<BondDownStatus> bondsWithCorrectionAccount) {
	...
	public void setTotalWithCorrectionWithoutTaxes(BigDecimal totalWithCorrectionWithoutTaxes) {
	...
	public void findBondsInErrorsCorrection() {
		SystemParameterService systemParameterService = ServiceLocator.getInstance()
				.findResource(SystemParameterService.LOCAL_NAME);
		correctionBondStatus = systemParameterService.materialize(MunicipalBondStatus.class,
		...
		String query = "SELECT mb.number as mbNumber," + "mb.emisiondate as emission,"
				+ "res.identificationnumber as resIdentification," + "res.name as resName," + "ent.name as entry,"
				+ "mb.value as value," + "mb.taxesTotal as taxes," + "stc.date as changeStatus,"
				+ "stc.explanation as explanation," + "usr.name as userName " + "FROM gimprod.municipalbond mb "
				...
				+ "LEFT JOIN gimprod.entry ent On ent.id = mb.entry_id " + "where stc.municipalbondstatus_id = :status "
				+ "and stc.previousbondstatus_id = 3 " + "and stc.date BETWEEN :startDate and :endDate "
		...
		bondsWithCorrectionDTO = NativeQueryResultsMapper.map(q.getResultList(),
				MunicipalBondErrorsCorrectionDTO.class);
		...
		String query2 = "SELECT  mb.number as mbNumber, " + "ent.name as entry, " + "itm.total as valueTotal, "
				+ "acc.accountcode as accountCode, " + "acc.accountname as accountName, "
				+ "itm.ordernumber as orderNumber " + "FROM gimprod.item itm  "
				...
				+ "where stc.municipalbondstatus_id = :status " + "and stc.previousbondstatus_id = 3  "
		...
		bondsWithCorrectionDetailDTO = NativeQueryResultsMapper.map(q2.getResultList(),
				MunicipalBondErrorsCorrectionDetailDTO.class);
	}
	...
	public void findBondsInErrorsCorrectionByAccount() {
		...
		String sqlNative = "select ac.accountcode, ac.accountname, sum(i.total) from gimprod.StatusChange sc "
				+ "left join gimprod.municipalbond mb " + "on mb.id=sc.municipalbond_id " + "left join gimprod.item i "
				+ "on i.municipalbond_id=sc.municipalbond_id " + "left join gimprod.entry e " + "on e.id = i.entry_id "
				+ "left join gimprod.account ac " + "on ac.id = e.account_id " + "where sc.municipalbondstatus_id ="
				+ correctionBondStatus.getId() + " " + "and sc.previousbondstatus_id = 3 " + "and sc.date between '"
				+ dateToStr(correctionStartDate) + "' AND '" + dateToStr(correctionEndDate) + "' "
				+ "and mb.emisionDate >= '" + dateToStr(userSession.getFiscalPeriod().getStartDate()) + "' "
				+ "group by ac.accountcode, ac.accountname " + "order by ac.accountCode";
		...
		sqlNative = "select ac.accountcode, ac.accountname, sum(ti.value) from gimprod.StatusChange sc "
				...
				+ "left join gimprod.taxitem ti " + "on ti.municipalbond_id=sc.municipalbond_id "
				+ "left join gimprod.tax t " + "on t.id = ti.tax_id " + "left join gimprod.account ac "
				+ "on ac.id = t.taxaccount_id " + "where sc.municipalbondstatus_id =" + correctionBondStatus.getId()
				+ " " + "and sc.previousbondstatus_id = 3 " + "and sc.date between '" + dateToStr(correctionStartDate)
				+ "' AND '" + dateToStr(correctionEndDate) + "' " + "group by ac.accountcode, ac.accountname "
				+ "order by ac.accountCode ";
	...
	@SuppressWarnings("unchecked")
	public void reverseStatusChangeDown(String resolution) {
		EntityManager entityManager = getEntityManager();
		Query query = entityManager.createNamedQuery("StatusChange.findStatusChangeDownByResolution");
		query.setParameter("resolution", resolution);
		List<StatusChange> list = query.getResultList();
		for (StatusChange statusChange : list) {
			statusChange.getMunicipalBond().setReversedDate(null);
			statusChange.getMunicipalBond().setReversedResolution(null);
			statusChange.getMunicipalBond().setReversedTime(null);
			statusChange.getMunicipalBond().setMunicipalBondStatus(statusChange.getPreviousBondStatus());
		}
		persist();
		String sql = "delete from gimprod.statuschange where explanation = '" + resolution
				+ "' and municipalbondstatus_id = 9;";
		query = entityManager.createNativeQuery(sql);
		query.executeUpdate();
		sql = "delete from gimprod.statuschange where explanation is null and user_id is null and municipalbond_id is null;";
		query = entityManager.createNativeQuery(sql);
		query.executeUpdate();
		if (viewByResolution) {
			loadBondDownStatus();
