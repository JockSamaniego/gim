...
public class RevenueServiceBean implements RevenueService {
	...
	@Override
	public Resident findResidentByIdentificationNumber(String identificationNumber) throws NonUniqueIdentificationNumberException{
	...
	public MunicipalBond createMunicipalBond(
		...
		return municipalBondService.createMunicipalBond(resident, fiscalPeriod, entry, entryValueItem, isEmission, true, facts);
	}
	
	/*
	public MunicipalBond createDeferredMunicipalBond(Resident resident,
			FiscalPeriod fiscalPeriod, Entry entry,
			BigDecimal base, Date serviceDate, Date expirationDate, String reference, String description,
			Object ... facts) throws EntryDefinitionNotFoundException{
		
		return municipalBondService.createDeferredMunicipalBond(resident, fiscalPeriod, entry, base, 
				serviceDate, expirationDate, reference, description, facts);
	}
	*/
	...
	/* (non-Javadoc)
	 * @see org.gob.gim.revenue.facade.RevenueService#emit(ec.gob.gim.revenue.model.MunicipalBond)
	 */
	@Override
		...
		if(canUserEmit(user)){
			...
			if(municipalBond.getId() != null){
				...
				if(municipalBond.getAdjunct() != null){
					System.out.println("EMITIENDO CON ADJUNTO "+municipalBond.getAdjunct()+ " CODIGO ADJUNTO "+municipalBond.getAdjunct().getCode());
	...
	public void emit(Long emissionOrderId, Long userId, Date startDate){
		...
		for(MunicipalBond municipalBond : loadMunicipalBonds(emissionOrderId)){
			...
			//rfarmijosm 2016-01-30 se comenta no es necesario buscar cada rato q se emite
			//municipalBond.setEmisionPeriod(fiscalPeriodService.findCurrent(now).get(0).getStartDate());
		}
	...
	public List<MunicipalBond> findMunicipalBondByResidentIdAndTypeAndStatus(
		...
		return municipalBondService.findByResidentIdAndTypeAndStatus(residentId, municipalBondType, municipalBondStatusId);
	}
	...
	/*
	@Override
	public void calculeMunicipalBond(MunicipalBond mb) {
	...
	*/
	...
	private void saveStatusChangeRecord(String explanation, MunicipalBond bond, MunicipalBondStatus previousStatus,MunicipalBondStatus status, User user){
	...
	/*
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public void updateReversedDescriptionAndValue(List<Long> selected, String observation) {
		Query query = entityManager.createNamedQuery("MunicipalBond.updateReverseDescriptionAndValue");
	...
	*/
	...
	public List<MunicipalBond> findMunicipalBondsForReverseByEmitter(Long residentId, Long emitterId, Date date){
		if(residentId != null){
			...
			Query query = entityManager.createNamedQuery("MunicipalBond.findMunicipalBondByResidentIdAndEmitterAndDate");
	 ...
	 */
	public void changeFutureToPendign(List<MunicipalBond> municipalBonds,User user, Person person, String explanation) throws Exception {
		...
		for (MunicipalBond municipalBond : municipalBonds) {
			...
			if (canUserEmit(user)) {
				...
				municipalBond.setMunicipalBondStatus(pendingBondStatus);
				municipalBond.setExpirationDate(getExpirationDateChangeFutureStatusToPendingStatus(now, municipalBond.getEmisionDate(), municipalBond.getExpirationDate()));
				municipalBond.setNumber(findNextMunicipalBondValue());	
				municipalBond.setEmisionDate(now);
				municipalBond.setEmisionPeriod(fiscalPeriodService.findCurrent(now).get(0).getStartDate());
	...
	public static Date getExpirationDateChangeFutureStatusToPendingStatus(Date now, Date emissionDate, Date expirationDate) {
