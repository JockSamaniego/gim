...
public class RevenueServiceBean implements RevenueService {
	...
	@Override
	public Resident findResidentByIdentificationNumber(
	...
	public MunicipalBond createMunicipalBond(Resident resident, Entry entry,
			FiscalPeriod fiscalPeriod, EntryValueItem entryValueItem,
			boolean isEmission, Object... facts)
	...
	private Boolean canUserEmit(User user) {
		String emisorRoleName = systemParameterService
				.findParameter("ROLE_NAME_EMISOR");
		String preemisorRoleName = systemParameterService
				.findParameter("ROLE_NAME_PREEMISOR");
	...
	private MunicipalBondType findMunicipalBondTypeByRole(User user) {
		...
		String preemisorRoleName = systemParameterService
				.findParameter("ROLE_NAME_PREEMISOR");
		String simertRoleName = systemParameterService
				.findParameter("ROLE_NAME_SIMERT");
		String urbanRoleName = systemParameterService
				.findParameter("ROLE_NAME_URBAN_EMISOR");
		String ruralRoleName = systemParameterService
				.findParameter("ROLE_NAME_RURAL_EMISOR");
		String solarRoleName = systemParameterService
				.findParameter("ROLE_NAME_SOLAR_EMISOR");
		if ((user.hasRole(emisorRoleName)) || (user.hasRole(simertRoleName))
				|| (user.hasRole(urbanRoleName))
				|| (user.hasRole(ruralRoleName))
				|| (user.hasRole(solarRoleName))) {
	...
	public void checkEmission(MunicipalBond municipalBond)
			throws InvalidEmissionException {
	 ...
	 * 
	 * @see
	 * org.gob.gim.revenue.facade.RevenueService#emit(ec.gob.gim.revenue.model
	 * .MunicipalBond)
	 */
	@Override
	public MunicipalBond emit(MunicipalBond municipalBond, User user)
			throws Exception {
		...
		if (canUserEmit(user)) {
			...
			MunicipalBondStatus previousStatus = municipalBond
			...
			if (municipalBond.getId() != null) {
				...
				saveStatusChangeRecord(null, municipalBond, previousStatus,
				...
				if (mt == MunicipalBondType.EMISSION_ORDER) {
					emissionOrderService.createEmissionOrder(municipalBond,
							user);
				}
			}
	...
	private List<MunicipalBond> loadMunicipalBonds(Long id) {
		Query query = entityManager
				.createNamedQuery("EmissionOrder.findMunicipalBondsByEmissionOrderId");
	...
	private Long findNextMunicipalBondValue() {
		Query query = entityManager
				.createNativeQuery("SELECT nextval('municipalBondNumber')");
		return ((BigInteger) query.getResultList().get(0)).longValue();
	}

	@Override
	public void emit(List<MunicipalBond> municipalBonds, User user)
			throws Exception {
	...
	@Override
	public List<MunicipalBond> findMunicipalBondByResidentIdAndTypeAndStatus(
			Long residentId, MunicipalBondType municipalBondType,
			Long municipalBondStatusId) {
		return municipalBondService.findByResidentIdAndTypeAndStatus(
				residentId, municipalBondType, municipalBondStatusId);
	}

	@Override
	public List<EntryStructure> findEntryStructureChildrenByType(
			Long entryParentId, EntryStructureType entryStructureType) {
		return entryService.findEntryStructureChildrenByType(entryParentId,
				entryStructureType);
	}
	...
	public void update(List<Long> municipalBondIds, Long municipalBondStatusId,
		...
		Long reversedStatusId = systemParameterService
				.findParameter("MUNICIPAL_BOND_STATUS_ID_REVERSED");
		...
		/*
		 * Query query =
		 * entityManager.createNamedQuery("MunicipalBond.changeStatus");
		 * query.setParameter("municipalBondIds", municipalBondIds);
	...
	private void saveStatusChangeRecord(String explanation, MunicipalBond bond,
	 ...
	 * updateReversedDescriptionAndValue(List<Long> selected, String
	 * observation) { Query query = entityManager.createNamedQuery(
	...
	@SuppressWarnings("unchecked")
	public List<MunicipalBond> findMunicipalBondsForReverseByEmitter(
			Long residentId, Long emitterId, Date date) {
		if (residentId != null) {
			MunicipalBondStatus pendingBondStatus = systemParameterService
					.materialize(MunicipalBondStatus.class,
							"MUNICIPAL_BOND_STATUS_ID_PENDING");
	 ...
	 * Emitir años futuros, la primer obligacion pertenece a este anio, las otras quedan en el aire, hasta pasarlas a la fecha q les corresponden
	 */
	public void emitFuture(List<MunicipalBond> municipalBonds, User user) throws Exception {
		MunicipalBondStatus futureBondStatus = systemParameterService.materialize(MunicipalBondStatus.class, "MUNICIPAL_BOND_STATUS_ID_FUTURE_EMISION");
		MunicipalBondStatus pendingBondStatus = systemParameterService .materialize(MunicipalBondStatus.class, "MUNICIPAL_BOND_STATUS_ID_PENDING");
		...
		for (MunicipalBond municipalBond : municipalBonds) {
			...
			if (canUserEmit(user)) {
				...
				if ( yearOfDate(municipalBond.getServiceDate()) > yearOfDate(now)  ) {
					...
					/*}else{
						//System.out.println("se mantiene normal "+ i);
						municipalBond.setEmisionDate(now);
						municipalBond.setEmisionTime(now);
						municipalBond.setMunicipalBondStatus(pendingBondStatus);
						//TODO identificar q numero va a tener sino enviar null y generar la proxima vez
						municipalBond.setNumber(findNextMunicipalBondValue());
						municipalBond.setEmisionPeriod(fiscalPeriodService.findCurrent(now).get(0).getStartDate());
					}*/
				}else{
	...
	/**
	 * @author Rene Ortega
	 * Cambiar de estado Municipal Bonds de anios futuros a Pendientes para poder cobrarlas
	 */
	public void changeFutureToPendign(List<MunicipalBond> municipalBonds,
		...
		for (MunicipalBond municipalBond : municipalBonds) {
			...
			if (canUserEmit(user)) {
				...
				MunicipalBondStatus previous = municipalBond
						.getMunicipalBondStatus();
	...
	public static Date getExpirationDateChangeFutureStatusToPendingStatus(
			Date now, Date emissionDate, Date expirationDate) {
		...
		diferenceDays.setTimeInMillis(lastDate.getTimeInMillis()
