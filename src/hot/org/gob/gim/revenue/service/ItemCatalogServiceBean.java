...
public class ItemCatalogServiceBean implements ItemCatalogService {
	...
	public List<ItemCatalog> findItemsForCatalogCodeOrderById(String catalogCode) {
		Query query = entityManager
				.createQuery("select i from ItemCatalog i where i.catalogCode=:catalogCode and i.code not in('INITIATED', 'SENT_ISSUE', 'NULLIFIED_ARCHIVED', 'FILE_NULLIFIED', 'PAID_ARCHIVED') ORDER BY i.id ASC");
		query.setParameter("catalogCode", catalogCode);
		List<ItemCatalog> listResult = query.getResultList();
		return listResult;
	}
	...
	public List<ItemCatalog> findItemsForSentStatus(String catalogCode) {
		Query query = entityManager
				.createQuery("select i from ItemCatalog i where i.catalogCode=:catalogCode and i.code in ('FILE_NULLIFIED','PAID_ARCHIVED') ORDER BY i.id ASC");
		query.setParameter("catalogCode", catalogCode);
		List<ItemCatalog> listResult = query.getResultList();
		return listResult;
	}
